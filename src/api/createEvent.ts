        ...
        timelines: [
            {
                ...
                events: [
                    {
                        eventId: "b8f7919b-5a10-0d82-7fc2-2c822c7aaaaa",
                        eventName: "超新しいイベント",
...
export const getTimeline = (story: Story, timelineId: string) => {
    return story.timelines.find((timeline) => timeline.timelineId === timelineId);
}

export const createEvent = (storyId: string, timelineId: string, event: Event) => {
    const story = getStory(storyId);
    if (story === undefined) {
        return undefined;
    }
    const timeline = getTimeline(story, timelineId);
